...
export class PlayerClient extends Player
{
    ...
    {
        if (this.bind && (key === this.bind.up || key === this.bind.down || key === this.bind.left || key === this.bind.right))
            return true;
        return false;
    }
    ...
    {
        if (event.key === " " && !this.isReady)
            this.ready();
        if (this.bind && this.isValidInput(event.key) && this.lastInput !== "+") {    
            switch (event.key) {
                case this.bind.up:
